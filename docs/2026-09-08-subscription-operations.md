...
기준일·공식 문서 최초 열람일: 2026-09-08 (Asia/Seoul). 이번 문서 정리는 코드/저장 계약 대조이며 외부 결제 정책 재조사는 아니다. 실제 연결 시 공식 정책을 다시 확인한다. 이번 사용자 방향은 **트레이너 1명이 운영**하는 앱이다. 여러 트레이너의 입점·승인·정산 도구는 범위에서 제외한다. 기존의 완성 프로그램 선택 및 운동·세트 구성 보존 결정을 유지한다.
...
현재 구현·검증·후속 범위는 [상태표](2026-09-08-remaining-work.md), 검토 후 수정은 [안정화 기록](2026-09-08-review-fixes.md)을 따른다.
...
| 기본 휴식 시간 | 미지정 또는1~3600초. 처방이 미지정인 새 휴식의 기본값 | 처방0초·양수를 덮지 않고 기존 타이머 시간을 바꾸지 않음 |
| 완료 후 자동 휴식 | 초기값 꺼짐; 켜면 최초 완료의 영구 저장 성공 뒤 시작 | 기존 완료 수정·제외·초안은 새 시작 없음. 기존 타이머 교체 확인과 재시도 계약은 [휴식 명세](2026-09-08-rest-guides.md) 및 안정화 기록 참조 |
...
- `AppSettings.defaultWeightUnit`: `WeightUnit.kg`가 초기값. `showLoadSuggestions`: 초기값 `true`. `defaultRestSeconds`는 초기값 `null`(미지정) 또는1~3600초 정수, `autoStartRestTimer`는 초기값 `false`다. 사용자 기본 시간은 처방 미지정에만 적용하고 처방0초는 휴식 없음으로 유지한다.
- `LocalSettingsStore`는 envelope1/2를 읽고 `schemaVersion: 2`로 쓴다. 옛1의 누락된 휴식 설정은 미지정·자동 시작 꺼짐으로 읽으며, 형식과 맞지 않는 필드는 오류로 처리한다. 일반 앱 진입은 운동 저장 폴더의 `app-settings.json`에 저장하고, 테스트처럼 운동 컨트롤러를 외부 주입한 진입은 해당 운동 파일 경로 뒤에 `.settings.json`을 붙여 격리한다. 저장소는 파일이 없을 때만 기본값을 반환한다. 손상/미지원 스키마/알 수 없는 단위/잘못된 bool은 오류로 보존한다.
- 읽기·쓰기는 인스턴스의 큐로 직렬화한다. 임시 파일 flush 후 rename하며 실패한 쓰기가 후속 재시도를 막지 않는다. 여러 프로세스 동시 쓰기나 전원 차단 검증을 뜻하지 않는다.
- `SettingsController.settings`는 마지막으로 읽기·저장에 성공한 적용값이며, 성공 이력이 없으면 초기값 kg/제안 표시 켜짐/휴식 미지정/자동 시작 꺼짐이다. `displayedSettings`는 미저장 선택을 포함한다. 저장 중 중복 변경은 막고, 실패 시 선택·오류를 유지한다. 재시도 성공 시에만 다른 화면에 적용한다.
...
- **설정 읽기 실패의 현재 동작:** 컨트롤러가 읽기 오류를 보유하고 원본 파일을 유지하며, `RootGate`는 이 오류만으로 앱 진입을 막지 않는다. 설정 화면에는 ‘설정을 불러오지 못했어요’와 ‘다시 불러오기’만 표시하고 수정 컨트롤을 숨긴다. 다른 화면은 컨트롤러에 남은 적용값을 사용하므로 최초 읽기 실패라면 새 입력은 kg, 제안 표시는 켜짐, 휴식은 미지정, 자동 시작은 꺼짐으로 동작한다. 같은 컨트롤러에 이전 성공값이 있으면 그 값을 유지한다. 홈 전체에 설정 오류 배너를 따로 표시하지는 않는다. 읽기 실패 후 설정 화면의 오류·원본 보존·재시도는 단독 위젯 테스트로 확인했고, 실패 상태에서 앱 진입과 새 입력으로 이어지는 대체값 경로는 코드 확인 범위다.

## 구독 운영 제안 — 현재 판매·정책 확정 아님

이하 상품·서버·지원·시험 계획은 최초 열람 자료를 토대로 남긴 운영 설계다. 실제 상품/계정/문의 채널은 미연결이며 로컬 설정이 구매 권한을 대신하지 않는다.
