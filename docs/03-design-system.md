...
## 9/8 고급 처방·휴식 타이머 구성 요소

Console v2의 같은 토큰과 기록 흐름을 유지하며 관리자에서 작성한 지시를 소비자에게 전달한다. 새 색·간격·폰트 토큰은 추가하지 않았다.

| 구성 요소 | 역할·상태·접근 |
|---|---|
| 고급 세트 설정 | `DetailedRoutineScreen`의 접힌 요약과 펼친 휴식/템포/AMRAP 입력. 휴식 원문·미지정/0초, 4구간 안내, AMRAP 기준 반복과 RIR 독립. 넓은 화면 3열·좁은 화면 세로 |
| 운동 묶기 · 슈퍼세트 | 운동 수준 그룹 ID, 같은 세션 연속 2개 이상 안내, 묶음 없음/지정 상태. 그룹 ID에는 한글도 가능하므로 KR 스타일 |
| 고급 변경 검토 | 기존 세트 처방 비교에 새 값 포함, 운동 그룹 변경 수와 전후 표기를 별도 표시. 확정 실패 때 같은 후보 재시도 |
| `repetitionSpans` / `repetitionLabel` | 고정 반복과 AMRAP 기준 반복을 구분한다. 사용자 AMRAP 목표 행은 중량과 반복을 곱셈으로 묶지 않고 구분점으로 분리 |
| `SetPrescriptionNotes` | 프로그램 상세·시작 전 확인·검색·운동 행/시트에서 휴식과 템포 표시. 시트·검색에서는 템포 순서와 AMRAP 기준/실제의 차이 설명. 숫자/템포 코드는 Mono, 설명은 KR |
| `SupersetNote` | 같은 그룹 운동을 한 세트씩 번갈아 진행한다는 안내. 운동 목록은 보존하고 다음 세트 이동만 라운드 순서를 사용 |
| 세트 휴식 시작 | 양수 휴식 처방의 저장된 완료 세트에서 명시 버튼. 0초/미지정·미완료·초안·보관/마감 상태에 시작 버튼 없음. 기록 미저장·타이머 오류/읽기 중에는 비활성 |
| `RestTimerPanel` | 운동 화면 상단의 한 개 타이머. 진행·일시정지·시간 종료, 남은 시간 Mono·출처 운동/세트 KR, 일시정지/재개/종료/확인 버튼. 종료는 다음 세트를 자동 완료하지 않음 |
| 타이머 교체 확인 | 현재 타이머의 운동/세트와 교체 의미, 취소/타이머 바꾸기. 저장 성공 뒤 교체 |
